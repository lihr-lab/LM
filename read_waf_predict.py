# read_waf.py - 筛选Jira站点的异常日志（按GET/POST分别存储）
import re
import json
from collections import defaultdict

file_path = "D:\\browser\\waf_10.67.10.72.json-20260321"
attacks_output_file_get = "waf_classified_attacks_get.json"
attacks_output_file_post = "waf_classified_attacks_post.json"

# 攻击类型的tag
ATTACK_TAGS = {
    'waf_log_websec', 'waf_log_ddos', 'waf_log_arp',
    'waf_log_deface', 'waf_log_ipblock', 'waf_log_14acl'
}

# 需要保留的字段
KEEP_FIELDS = ['uri', 'method', 'alertlevel', 'event_type', 'http', 'raw_client_ip', 'stat_time']

def is_jira_log(line):
    return 'inone.intra.nsfocus.com' in line or '/jira/' in line

def extract_fields(line):
    """提取指定字段"""
    fields = {}
    
    # 提取 uri 或 url
    uri = re.search(r'uri:([^;]+?)(?=;\w+:|$)', line)
    if uri:
        fields['uri'] = uri.group(1).strip().strip('"')
    else:
        url = re.search(r'url:([^;]+?)(?=;\w+:|$)', line)
        if url:
            fields['uri'] = url.group(1).strip().strip('"')
    
    # method
    method = re.search(r'method:([^;]+?)(?=;\w+:|$)', line)
    if method:
        fields['method'] = method.group(1).strip()
    
    # alertlevel
    alertlevel = re.search(r'alertlevel:([^;]+?)(?=;\w+:|$)', line)
    if alertlevel:
        fields['alertlevel'] = alertlevel.group(1).strip()
    
    # event_type
    event_type = re.search(r'event_type:([^;]+?)(?=;\w+:|$)', line)
    if event_type:
        fields['event_type'] = event_type.group(1).strip()
    
    # http - 提取到行尾或遇到 ;http_protocol: 或 ;wsi:
    http_match = re.search(r'http:(.+?)(?=;http_protocol:|$)', line, re.DOTALL)
    if not http_match:
        http_match = re.search(r'http:(.+?)(?=;wsi:|$)', line, re.DOTALL)
    if not http_match:
        http_match = re.search(r'http:(.+)$', line)
    
    if http_match:
        http_val = http_match.group(1).strip()
        if http_val.endswith('"'):
            http_val = http_val[:-1]
        http_val = http_val.replace('#015#012', '\\r\\n')
        fields['http'] = http_val
    
    # raw_client_ip
    raw_client_ip = re.search(r'raw_client_ip:([^;]+?)(?=;\w+:|$)', line)
    if raw_client_ip:
        fields['raw_client_ip'] = raw_client_ip.group(1).strip()
    
    # stat_time
    stat_time = re.search(r'stat_time:([^;]+?)(?=;\w+:|$)', line)
    if stat_time:
        fields['stat_time'] = stat_time.group(1).strip()
    
    return fields

print("=" * 60)
print("开始筛选Jira异常日志（按GET/POST分别存储）...")
print(f"输出字段: {KEEP_FIELDS}")
print("=" * 60)

attacks_get = []   # GET 请求
attacks_post = []  # POST 请求
scan_count = 0
attack_count = 0
TARGET = 1000  # 每种类型最多收集100条

with open(file_path, 'r', encoding='utf-8', errors='replace') as f:
    for line_num, line in enumerate(f, 1):
        # 检查是否已达到目标数量
        if len(attacks_get) >= TARGET and len(attacks_post) >= TARGET:
            print(f"\n已达到目标数量（GET: {TARGET}, POST: {TARGET}），停止扫描。")
            break
        
        line = line.strip()
        if not line:
            continue
        
        scan_count += 1
        
        if scan_count % 5000 == 0:
            print(f"  已扫描 {scan_count} 行，已找到 GET: {len(attacks_get)} 条, POST: {len(attacks_post)} 条...")
        
        # 只筛选Jira日志
        if not is_jira_log(line):
            continue
        
        # 获取 tag
        tag_match = re.search(r'tag:([^;\s]+)', line)
        tag = tag_match.group(1) if tag_match else None
        
        # 只处理攻击日志
        if tag not in ATTACK_TAGS:
            continue
        
        # 对于 waf_log_14acl，只处理阻断的
        if tag == 'waf_log_14acl':
            if 'Block' not in line and '阻断' not in line:
                continue
        
        # 提取字段
        fields = extract_fields(line)
        
        # 只保留指定字段
        filtered_fields = {k: fields[k] for k in KEEP_FIELDS if k in fields}
        
        # 获取 method
        method = filtered_fields.get('method', '')
        
        # 根据 method 分类存储
        if method == 'GET' and len(attacks_get) < TARGET:
            attacks_get.append(filtered_fields)
            attack_count += 1
        elif method == 'POST' and len(attacks_post) < TARGET:
            attacks_post.append(filtered_fields)
            attack_count += 1

print(f"\n完成！")
print(f"  总扫描行数: {scan_count}")
print(f"  找到的攻击日志: {attack_count} 条")
print(f"    - GET 请求: {len(attacks_get)} 条")
print(f"    - POST 请求: {len(attacks_post)} 条")

# 保存 GET 请求
with open(attacks_output_file_get, 'w', encoding='utf-8') as f:
    json.dump(attacks_get, f, ensure_ascii=False, indent=2)

# 保存 POST 请求
with open(attacks_output_file_post, 'w', encoding='utf-8') as f:
    json.dump(attacks_post, f, ensure_ascii=False, indent=2)

print(f"\nGET 攻击日志已保存到: {attacks_output_file_get}")
print(f"POST 攻击日志已保存到: {attacks_output_file_post}")

# 验证
print("\n" + "=" * 60)
print("验证结果:")
if attacks_get:
    first_get = attacks_get[0]
    print(f"GET 请求示例 - 包含的字段: {list(first_get.keys())}")
    print(f"  uri: {first_get.get('uri', '无')[:80]}")
    print(f"  event_type: {first_get.get('event_type', '无')}")

if attacks_post:
    first_post = attacks_post[0]
    print(f"\nPOST 请求示例 - 包含的字段: {list(first_post.keys())}")
    print(f"  uri: {first_post.get('uri', '无')[:80]}")
    print(f"  event_type: {first_post.get('event_type', '无')}")

print("\n完成！")